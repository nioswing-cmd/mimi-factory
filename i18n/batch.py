...
def main():
    ...
    force = "--force" in sys.argv   # 원본 수정 후 언어판 재빌드용 (멱등 스킵 무시)
    if "--file" in sys.argv:
        # 단건 모드 — 새벽 생산 훅(신작 실시간 다국어화)용
        rel = sys.argv[sys.argv.index("--file") + 1].replace("\\", "/")
        slug = os.path.splitext(os.path.basename(rel))[0]
        queue = [] if (done_already(slug) and not force) else [(rel, slug)]
    else:
        queue = [(rel, slug) for rel, slug in build_queue()
                 if force or not done_already(slug)]
    if limit:
        queue = queue[:limit]
    ...
    # 🔴 실패했으면 실패했다고 끝내야 한다 (2026-08-12 발견).
    #    여태 main() 이 아무것도 안 돌려줘서 **종료코드가 늘 0** 이었다.
    #    부르는 쪽(build_from_sheet.localize_new)은 종료코드를 제대로 보고 있었는데,
    #    이쪽이 「성공 0 / 실패 1」 을 적어놓고도 0 을 돌려주는 바람에
    #    화면에는 늘 「🌏 다국어화 완료」가 떴다 — 실제로는 한 건도 안 만들어졌다.
    #    (연애편·설렘편 둘 다 ja/en/zh-tw 파일이 0개인데 「완료」로 보고됐다.
    #     게다가 실패할 때마다 번역 호출값이 그대로 나갔다 — 편당 $0.5~0.9)
