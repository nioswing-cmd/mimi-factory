...
def check_leftovers(html: str, extra_markers):
    ...
    default_markers = [
        ...
        "모리오리", "뉴기니",
        # 주: '안나 카레니나'는 총·균·쇠가 '안나 카레니나 법칙'으로 인용하는 표현이라
        # 원래 GGS 잔존 마커에 있었으나, 이 스킬로 실제 『안나 카레니나』 퀴즈를 빌드하면
        # 책의 정당한 본문·제목과 충돌(오탐)한다. 다른 GGS 마커(동서 축·얄리·재레드
        # 다이아몬드 등)로 잔존 검출력은 유지되므로 이 항목만 기본 마커에서 제외한다.
    ]
